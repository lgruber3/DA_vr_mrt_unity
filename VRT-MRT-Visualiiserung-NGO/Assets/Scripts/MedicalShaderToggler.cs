...
public class MedicalShaderToggle : NetworkBehaviour
{
    ...
    public void SelectDensity()          => ChangeMaterial(0);
    public void SelectClinical()         => ChangeMaterial(1);
    public void SelectSurfaceCurvature() => ChangeMaterial(2);

    private void ChangeMaterial(int index)
    {
        ApplyMaterial(index);
        if (IsHost) ApplyMaterialClientRpc(index);
    }
    ...
    private void ApplyMaterial(int index)
    {
        if (_boneRenderer == null)
        {
            ...
            if (bone != null) _boneRenderer = bone.GetComponent<Renderer>();
        }
        if (_tissueRenderer == null)
        {
            var tissue = AssetManager.Instance?.GetTissueAsset();
            if (tissue != null) _tissueRenderer = tissue.GetComponent<Renderer>();
        }

        switch (index)
        {
            case 0:
                if (_boneRenderer)   _boneRenderer.material   = boneDensityMat;
                if (_tissueRenderer) _tissueRenderer.material  = tissueDensityMat;
                break;
            case 1:
                if (_boneRenderer)   _boneRenderer.material   = boneClinicalMat;
                if (_tissueRenderer) _tissueRenderer.material  = tissueClinicalMat;
                break;
            case 2:
                if (_boneRenderer)   _boneRenderer.material   = boneSurfaceCurvatureMat;
                if (_tissueRenderer) _tissueRenderer.material  = tissueSurfaceCurvatureMat;
                break;
        }
    }
}
